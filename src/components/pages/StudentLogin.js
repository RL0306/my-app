...
const StudentLogin =  () => {
  ...
  const handleLogin = async (e) => {
    ...
    try{
      const req = await axios.post("http://localhost:8080/api/v1/student/login", {username : formData.get("user_id"), password : formData.get("password")}, {withCredentials : true});
      const data = await req.data;
      if(data.authentication === "success"){
        // console.log("in?")
        setOutput("Successfully logged in. Redirecting you to home!");
        const userLoggedInReq = await axios.get("http://localhost:8080/api/v1/student", {withCredentials : true})
        const userData = await userLoggedInReq.data;
